...
#: Приказы, которые отдаёт ГМ. Паника сюда не входит: это не команда, а
#: состояние, в которое элемент срывается сам, когда мораль пробивает порог.
#: Пока она была в выпадающих списках, её можно было «приказать» — и тогда
#: у стороны не оставалось выполнимой задачи (`task.kind: none`), элемент
#: продолжал стрелять, а через ход приказ молча переписывался на отступление.
COMMAND_ORDERS: tuple[Order, ...] = tuple(order for order in Order if order != Order.PANIC)
